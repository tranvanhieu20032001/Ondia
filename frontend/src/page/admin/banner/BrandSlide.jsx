...
function BrandSlide() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (banners.slide.length > 3) {
        alert("Đã đủ 4 ảnh thương hiệu! Không thể tải ảnh mới.");
        return;
      }
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4 text-gray-700">Quản lý giới thiệu</h1>
      {banners.slide.length <= 3 && (<p className="text-red-500 my-4">*Chú ý phải tải đủ 4 ảnh</p>)}
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex items-center space-x-4">
          <div className="flex-1 relative">
            ...
            {preview && (
              <div className="mt-2 flex justify-center items-center h-[200px] w-[200px] relative border rounded overflow-hidden">
                <img
                  src={preview}
                  alt="Banner preview"
        ...
        <div className="flex flex-wrap gap-4">
          {banners.slide.length > 0 ? (
            banners.slide.map((banner, index) => (
              <div key={index} className="w-[250px] h-[250px] flex justify-center items-center relative">
                <img
